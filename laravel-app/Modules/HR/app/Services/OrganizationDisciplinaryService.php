...
use App\Jobs\IncentiveDisciplinary\DisciplinaryActionExportToExcelJob;
use App\Models\UserExportTask;
use Modules\HR\Models\OrganizationDisciplinary;

class OrganizationDisciplinaryService
{
    public function list(array $filters, $user)
    {
        if (!empty($filters['download'])) {
            $task = UserExportTask::create([
                'user_id' => $user->id,
                'type' => ExportTaskEnum::DISCIPLINARY->value,
            ]);

            DisciplinaryActionExportToExcelJob::dispatch($filters, $task, $user);
            return null;
        }

        return OrganizationDisciplinary::query()
            ->filter($user, $filters)
