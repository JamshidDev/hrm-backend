...
class TurnstileLateWorkersExportToExcelJob implements ShouldQueue
{
    use Queueable;
            ...
            $query = TurnstileWorkerSchedule::query()
                ...
                ->leftJoin('worker_positions as wp', function ($join) use ($user) {
                    $join->on('turnstile_worker_schedules.worker_position_id', '=', 'wp.id')
                        ->whereIn('wp.id', WorkerPosition::filter($user, $this->query)->select('id'))->where('wp.status', PositionStatusEnum::ACTIVE->value);
                })
                ...
                ->whereIn('turnstile_worker_schedules.worker_position_id', function ($subquery) use ($user){
                    $subquery->select('id')
                        ...
                        ->whereIn('id', WorkerPosition::filter($user, $this->query)->select('id'))
                ...
                ->get()->map(function ($item){
                    return [
                        ...
                        'delay_minutes' => round($item->delay_minutes),
