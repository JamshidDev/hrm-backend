...
describe('LmsEduPlanExamService', () => {
  describe('attach', () => {
    it('exam_type default 3 (Final)', async () => {
      ...
      const svc = new LmsEduPlanExamService(
        db,
        {} as any,
        { t: (k: string) => k } as any,
  ...
  describe('results', () => {
    it('Bo`sh natija — paginatsiya shapeni qaytaradi', async () => {
      // lmsPaginate: select #1 = count ([{total}]), select #2 = list (rows).
      const { db } = buildMockDb({ selectResults: [[{ total: 0 }], []] });
      const svc = new LmsEduPlanExamService(
        db,
        { lang: 'uz' } as any,
        { t: (k: string) => k } as any,
      ...
      expect(r).toEqual({ current_page: 2, total: 0, data: [] });
    });
  });
});
